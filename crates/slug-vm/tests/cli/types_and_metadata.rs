...
#[test]
fn body_derived_known_calls_preserve_static_and_dynamic_boundaries() {
    let path = fixture_path("body-derived-call-inference");
    fs::write(
        &path,
    ...
    .expect("write statically incompatible inferred call");
    let output = slug()
        .arg(&path)
        .output()
    ...
    .expect("write dynamic inferred call");
    let output = slug()
        .arg(&path)
        .output()
    ...
    fs::remove_file(path).expect("remove body-derived call source");
}

#[test]
fn body_derived_nil_guards_preserve_static_and_dynamic_boundaries() {
    ...
    for source in [
        ...
        fs::write(&path, source).expect("write statically incompatible nil-guard call");
        let output = slug()
            .arg(&path)
            .output()
            .expect("run statically incompatible nil-guard call");
        assert_eq!(output.status.code(), Some(1));
        assert!(output.stdout.is_empty());
        assert!(
            String::from_utf8(output.stderr)
    ...
    .expect("write dynamic nil-guard call");
    let output = slug()
        .arg(&path)
        .output()
        .expect("run dynamic nil-guard call");
    assert_eq!(output.status.code(), Some(1));
    assert!(output.stdout.is_empty());
    assert!(
        String::from_utf8(output.stderr)
    ...
    fs::remove_file(path).expect("remove nil-guard source");
}
