...
| Integer bitwise, shift, and prefix `~` operators                           | Implemented                 | Source lowering and checked VM operations, plus CLI/VM tests                  | Keep list-concatenation and pipeline as separate slices.                     |
| List-concatenation and pipeline operators                                  | Not implemented             | Lexer/parser/bytecode have no representations                                 | Add one operator family at a time with type-failure tests.                   |
