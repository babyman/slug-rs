...
- [ ] Add CLI coverage for a direct wrapper:
...
- [ ] Add CLI coverage for `if (value == nil) { 0 } else { value / 10 }` and
  the reversed `!= nil` shape. Assert that `nil` and a number succeed, while a
  statically known string is rejected.
- [ ] Preserve a dynamic-call regression: a dynamic value passed to either
...
- [ ] Add focused semantic unit tests for inferred parameter and result types;
  avoid asserting private representations through public CLI output.

## 2. Add known-direct-call constraints

- [ ] Make parameter-constraint collection aware of the lexical semantic
  environment and already-published callable signatures.
- [ ] Bind ordinary positional and named arguments with the same shape rules as
  normal call resolution, then constrain a direct unannotated parameter by the
  uniquely resolved callee input type.
- [ ] Use an inferred alternative only when its selection is unique; otherwise
  retain the ordinary dynamic boundary.
- [ ] Infer the wrapper result from the selected callee result while preserving
  the existing result-flow rules.
- [ ] Stage signature collection before strict body-call validation so
  `divide(value)` can establish `value:num` rather than fail as `any|nil`.
- [ ] Do not obtain a callee constraint from dynamic/structural values, spread
...
- [ ] Represent a direct parameter-to-`nil` comparison as positive and negative
  input partitions alongside existing flow facts.
- [ ] Collect numeric and callable constraints in the narrowed branch only;
  join each reachable parameter partition after the conditional.
- [ ] Include the `nil` partition even when its branch terminates: it remains a
  valid input domain for the function.
- [ ] Support the direct `== nil` and `!= nil` forms first. Leave aliases,
  computed comparisons, mutation-sensitive bindings, and arbitrary predicates
  conservative until separately specified.
- [ ] Diagnose a contradiction only when a reachable partition has provably
  incompatible requirements. Otherwise widen incomplete knowledge.

## 4. Define and enforce recursion boundaries

- [ ] Preserve existing `recur` checking against the enclosing signature after
  its body-derived facts are solved.
- [ ] Cover a wrapper that calls a previously declared function and an exported
  imported callable whose signature snapshot is available.
- [ ] Add regressions showing that direct self-reference and mutually recursive
  ...
  behavior.
- [ ] Do not broaden inferred-alternative preservation through structural
  function values as part of this work.

## 5. Verify frontend and module boundaries

- [ ] Add module-loader coverage that imports an exported numeric function and
  proves a local wrapper receives its parameter/result precision.
- [ ] Add CLI coverage for public diagnostics and a source span at the wrapper
  call that is statically incompatible.
- [ ] Exercise REPL/server compilation if its retained session snapshots use a
  distinct semantic-entry path.
- [ ] Run the narrow semantic, CLI, and module-loader tests while iterating;
  run `make check` before completion.

## 6. Complete the language record when implementation lands

- [ ] Update the support inventory and regenerate the language support matrix.
- [ ] Update the README capability statement and `changelog.md` under
  `Unreleased` to state the implemented boundary.
- [ ] Keep the language specification and this ADR aligned with the final
